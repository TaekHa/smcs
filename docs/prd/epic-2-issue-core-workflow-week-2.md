...
- 4: 검색: 제목/내용 부분일치(ILIKE) + 발신자 전화번호 **정확매칭**(숫자 정규화 후 HMAC). 발신자명은 보안상 검색 미지원(MVP). [v0.3 정정 — 원문 "발신자명/전화번호 부분일치"는 발신자 PII 가 AES-GCM 암호화 컬럼이라 부분검색 불가. ADR-004 / architecture §5.3·§6.5 보안 SoT 반영. Story 2.2 PO 승인 Deviation #1.]
...
> 추가 경위: 원안에서 알림은 Story 2.5 AC6(미읽음 카운트) + 2.3/2.4 의 생성 부수효과로 흩어져 있었으나, 2.3/2.4/2.5/2.7 에 걸치는 횡단 모듈이라 PO 결정(2026-05-22)으로 단일 전용 스토리로 분리·신설.
...
- 1: 이슈 배정/코멘트/상태변경 시 관련자에게 인앱 알림이 트랜잭션 내 자동 생성된다(kind: `ISSUE_ASSIGNED`/`ISSUE_COMMENTED`/`ISSUE_STATUS_CHANGED`). 수신자 = {담당자, 접수자} − 행위자.
- 2: 헤더 벨 아이콘에 미읽음 카운트 뱃지가 표시되고, 30초 주기 폴링(보이는 탭만)으로 갱신된다.
- 3: 벨 탭 시 알림 리스트 페이지(`/notifications`)로 이동(최신순).
- 4: 알림 클릭 시 `readAt` 갱신 + 해당 이슈 상세로 이동.
- 5: "모두 읽음" 처리를 지원한다.
- 6: 알림은 본인(recipient) 것만 조회/처리 가능하다.
- 참고: `ISSUE_REOPENED` 알림은 Story 2.7(재오픈)에서 생성. 본 스토리 완료 시 Story 2.5 AC6 충족.
